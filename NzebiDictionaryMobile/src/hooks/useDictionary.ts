...
import { getDictionaryEntries, addDictionaryEntries, isDatabaseEmpty } from '../utils/sqliteDB';
  ...
  useEffect(() => {
    const loadDictionary = async () => {
      try {
        ...
        // Vérifier si SQLite contient déjà des données
        const isEmpty = await isDatabaseEmpty();
        
        if (!isEmpty) {
          // Charger depuis SQLite
          console.log('Chargement des données depuis SQLite...');
          const sqliteEntries = await getDictionaryEntries();
          setEntries(sqliteEntries);
          console.log(`${sqliteEntries.length} entrées chargées depuis SQLite`);
        } else {
          // Première utilisation : charger depuis le fichier JSON et sauvegarder dans SQLite
          console.log('Première utilisation : chargement depuis dictionary.json...');
          
          try {
            const jsonEntries: DictionaryEntry[] = dictionaryData as DictionaryEntry[];
            
            if (jsonEntries.length === 0) {
              throw new Error('Aucune donnée trouvée dans le fichier JSON');
            }

            // Sauvegarder dans SQLite pour les utilisations futures
            await addDictionaryEntries(jsonEntries);
            setEntries(jsonEntries);
            console.log(`${jsonEntries.length} entrées chargées depuis JSON et sauvegardées dans SQLite`);
          } catch (jsonError) {
